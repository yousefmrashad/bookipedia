...
from ..utils.config import DB_NAME, FETCHING_LIMIT, L1, L2, RERANKER_MODEL_NAME, SORT
from ..utils.functions import id_filter, ids_filter
...
class Weaviate(VectorStore):
    ...
    def similarity_search(
        self, query: str, source_ids: list, auto_merge=False, k=16, top_k=5, alpha=0.5
    ) -> list[Document]:
    ...
    def similarity_search_with_relevance_scores(
        self, query: str, source_ids: list, k=5, alpha=0.5
    ...
    def max_marginal_relevance_search(
        self, query: str, source_ids: list, k=5, fetch_k=20, lambda_mult=0.5
    ) -> list[tuple[Document, float]]:
        query_emb = self.embedder.embed_query(query)

        objects = self.collection.query.near_vector(
    ...
    def auto_merge(self, objects: list[Object], source_id: str) -> list[Object]:
        ...
        l0_chunks_keys = [
            key for key, value in l1_count.items() if (value <= (L1 // 2))
        ]
        l1_chunks_keys = [
            key for key, value in l1_count.items() if (value >= (L1 // 2) + 1)
        ]
        l2_chunks_keys = [
            key for key, value in l2_count.items() if (value >= (L2 // 2) + 1)
        ]
        ...
        l1_chunks_keys = [
            c
            for c in l1_chunks_keys
            if all(
                c not in range(p * l_ratio, (p + 1) * l_ratio) for p in l2_chunks_keys
            )
        ]
        ...
        if l1_chunks_keys:
            l1_filters = (
                wvc.query.Filter.by_property("l1").contains_any(l1_chunks_keys)
                & source_id_filter
            )
            l1_chunks = self.collection.query.fetch_objects(
                filters=l1_filters, limit=FETCHING_LIMIT, sort=SORT
            ).objects
        ...
        if l2_chunks_keys:
            l2_filters = (
                wvc.query.Filter.by_property("l2").contains_any(l2_chunks_keys)
                & source_id_filter
            )
            l2_chunks = self.collection.query.fetch_objects(
                filters=l2_filters, limit=FETCHING_LIMIT, sort=SORT
            ).objects
