...
from ..utils.functions import count_tokens, id_filter, merge_chunks, page_filter
from .weaviate_retriever import Weaviate
from .web_researcher import WebResearchRetriever
from .web_weaviate import WebWeaviate
...
class RAGPipeline:
    def __init__(self, embedding_model: Embeddings, client: WeaviateClient) -> None:
        ...
        self.llm = GoogleGenerativeAI(
            model=LLM_MODEL_NAME, google_api_key=GOOGLE_API_KEY, temperature=0
        )
        ...
        self.web_retriever = WebResearchRetriever.from_llm(
            vectorstore=self.web_db, llm=self.llm, search=self.search
        )
    ...
    async def summarize_text(self, text: str) -> AsyncIterator[str]:
        ...
        chunks_summaries = await asyncio.gather(
            *(self.summary_chain.ainvoke({"text": t}) for t in chunks)
        )
    ...
    async def summarize_pages(
        self, doc_id: str, page_nos: list[int]
    ) -> AsyncIterator[str]:
        pages_text = "\n\n".join(
            [self.get_page_text(doc_id, page_no) for page_no in page_nos]
        )
    ...
    def generate_chat_summary(
        self, user_prompt: str, llm_response: str, prev_chat_summary: str
    ):
    ...
    def generate_retrieval_query(
        ...
        retrieval_query_schema = ResponseSchema(
            name="retrieval_query", description="This is the retrieval query"
        )
    ...
    def generate_vecdb_context(
        self, retrieval_query: str, doc_ids: list[str]
    ) -> tuple[list[str], list[dict[str, str]]]:
        docs = self.db.similarity_search(
            query=retrieval_query, source_ids=doc_ids, auto_merge=True
        )
    ...
    def generate_web_context(
        self, retrieval_query: str, rerank=True
    ...
    def generate_context(
        ...
        if (retrieval_method in ("Retrieval", "retrieval", "Hybrid", "hybrid")) and (
            doc_ids
        ):
            vecdb_context, vecdb_metadata = self.generate_vecdb_context(
                retrieval_query, doc_ids
            )

        # Generate web context
        web_context, web_metadata = [], []
        if (retrieval_method in ("Web", "web", "Hybrid", "hybrid")) or (
    ...
    def generate_answer(
        self, user_prompt: str, chat: str, context: str
    ) -> AsyncIterator[str]:
        ...
        llm_input = {"user_prompt": user_prompt, "context": context, "chat": chat}
