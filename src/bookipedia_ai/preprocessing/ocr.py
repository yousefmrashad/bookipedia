...
from ..utils.config import DETECTION_MODEL, RECOGNITION_MODEL
from ..utils.functions import map_values
...
class OCR:
    ...
    @staticmethod
    def filter_image(
        img: np.ndarray, kernel_size=55, white_point=120, black_point=70
    ) -> np.ndarray:
        if kernel_size % 2 == 0:
            kernel_size += 1
    ...
    def correct_skew(img: np.ndarray) -> np.ndarray:
        ...
        corrected_image = cv.warpAffine(
            img, rotation_matrix, (cols, rows), flags=cv.INTER_LINEAR
        )
