...
from ..utils.functions import calculate_imagebox_percentage, count_tokens
...
class Document:
    ...
    def generate_embeddings(self, embedder: Embeddings):
        self.embeddings = embedder.embed_documents(
    ...
    def store_in_db(self, client: WeaviateClient):
        ...
        not_exist = (
            len((collection.query.fetch_objects(filters=exist_filter, limit=1).objects))
            == 0
        )
        ...
        if not_exist:
            ...
            for i, chunk in enumerate(self.chunks):
                ...
                obj = wvc.data.DataObject(
                    properties=properties, vector=self.embeddings[i]
                )
                objs.append(obj)
