...
from pikepdf import Matrix, Name, Pdf, Rectangle
...
class HocrTransform:
    ...
    def __init__(
        self,
        *,
        hocr: ET.ElementTree | str | Path = None,
        dpi: float,
        debug: bool = False,
        ...
        if isinstance(hocr, ET.ElementTree):
            self.hocr = hocr
        else:
            self.hocr = DefusedET.parse(os.fspath(hocr))
    ...
    def to_pdf(
        ...
        *,
        ...
        if image is not None:
            canvas.do.draw_image(image, 0, 0, width=self.width, height=self.height)
